...
export class SlashHandler {
  ...
  public async registCachedCommands(client: BotClient) {
    ...
    await rest.put(
      ...
      { body: [] }
    ).then(() => Logger.log('Successfully deleted commands.'));
    ...
    if (process.env.ENVIROMENT?.toUpperCase() === 'DEV') {
      await rest.put(
        ...
        { body: [] }
      ).then(() => Logger.log("Successfully deleted commands for guild: " + process.env.ENVIROMENT_DEV_GUILDID!));
      await rest.put(
        Routes.applicationGuildCommands(process.env.DISCORD_CLIENTID, process.env.ENVIROMENT_DEV_GUILDID!),
        { body: metadatas }
      ).then(() => Logger.log('Registered commands for guild: ' + process.env.ENVIROMENT_DEV_GUILDID!));
      return;
    }
    ...
    await rest.put(
      Routes.applicationCommands(process.env.DISCORD_CLIENTID),
      { body: metadatas }
    ).then(() => Logger.log('Registered commands.'));
  }
